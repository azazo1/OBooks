...
final class NativeReaderViewTests: XCTestCase {
    ...
    func testHorizontalPageLayoutUsesViewportOffsets() {
        let textView = ReaderTextView(frame: NSRect(x: 0, y: 0, width: 900, height: 600))
        textView.isEditable = false
        textView.isVerticallyResizable = true
        textView.isHorizontallyResizable = false
        textView.textStorage?.setAttributedString(NSAttributedString(
            string: String(repeating: "Horizontal page layout text. ", count: 700),
            attributes: [.font: NSFont.systemFont(ofSize: 18)]
        ))
        textView.setReadingInsets(horizontal: 56, vertical: 108)
        textView.configurePageColumns(
            2,
            viewportSize: NSSize(width: 900, height: 600),
            orientation: .horizontal,
            paginated: true
        )
        textView.updateDocumentSize(minimumSize: NSSize(width: 900, height: 600))

        guard let layoutManager = textView.layoutManager else {
            XCTFail("Missing layout manager")
            return
        }
        ...
        let thirdRange = layoutManager.characterRange(
            forGlyphRange: layoutManager.glyphRange(for: containers[2]),
            actualGlyphRange: nil
        )
        XCTAssertEqual(textView.pageOffset(forCharacter: firstRange.location) ?? -1, 0, accuracy: 1)
        XCTAssertEqual(textView.pageOffset(forCharacter: secondRange.location) ?? -1, 0, accuracy: 1)
        XCTAssertEqual(
            textView.pageOffset(forCharacter: thirdRange.location + 1) ?? -1,
            900,
            accuracy: 1,
            "third=\(thirdRange), first=\(firstRange), second=\(secondRange), containers=\(containers.count), frame=\(textView.frame)"
        )
        XCTAssertEqual(textView.frame.height, 600, accuracy: 1)
        XCTAssertGreaterThan(textView.frame.width, 900)
    }
}
