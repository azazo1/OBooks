...
生产环境可由反向代理终止 TLS, 将流量转发到回环监听地址. 如必须在容器中监听非回环 HTTP 地址, 使用 `--allow-http`, 并仅向可信的 TLS 代理开放该端口. 代理需要允许 512 MiB 的 EPUB 请求和较长的上传时间. 也可直接配置 `tlsCertificate` 与 `tlsKey` 启用 HTTPS. `/healthz` 提供进程健康检查, 不受错误请求限流影响. 同一客户端地址在 1 分钟内累计 20 次 4xx 响应后会被临时拒绝; 登录还单独限制每分钟次数和并发, 用于抑制扫描和口令尝试.

限流和日志按客户端真实地址计数. 反向代理需设置 `X-Forwarded-For` 头, 例如 nginx 的 `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`; 服务端配置 `trustedProxies` 声明允许覆盖该头的代理地址, 默认信任本机回环 (`127.0.0.1`, `::1`), 代理在非本机时填写其网段, 置为空数组则完全不信任. 请求未来自受信代理时该头会被忽略, 头中从右向左第一个不受信地址才被视为客户端地址, 因此客户端自行伪造的值不生效.

服务以单实例运行, 元数据保存在 SQLite WAL 数据库中, 书籍文件保存在 `objects/` 内. 使用专用操作系统用户启动服务, 持久化整个数据目录. 服务端能读取书籍和标注内容.

## 账号和设备

在书库侧栏打开云同步设置. 可同时保存多个账号, 切换时不会退出其他会话. 添加账号在独立窗口中填写, 登录成功后才写入本机账号列表. 已登录时可编辑设备名称或修改云端密码; 改密后其他设备需要重新登录. 删除本机账号会清掉这台电脑上的书库和登录状态, 并尽量撤销本机会话, 云端账号仍在. refresh token 以 0600 权限保存在书库目录的 `sync-refresh-token` 中, 同步文件不保存密码.
...
每份本地书库对应一个账号或未绑定目录. 登录新账号会切换到该账号自己的空书库, 不会自动带走当前书. 可从其他本地账号或未绑定书库复制过来: 按内容指纹合并书签, 高亮和阅读统计, 不覆盖已有正文, 也不删除来源.
